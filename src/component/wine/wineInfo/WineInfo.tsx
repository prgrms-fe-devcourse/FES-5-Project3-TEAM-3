...
function WineInfo({ wineInfo }: { wineInfo: WineInfoType }) {
  ...
  const computedTaste = Object.fromEntries(
    Object.entries(taste).map(([k, v]) => {
      return v === null ? [k, null] : [k, Math.round(v * 0.05)];
    })
  ) as typeof taste;

  return (
    <div className="min-w-90 w-100 h-120 flex flex-col gap-2 px-12 py-8 m-8 items-stretch mx-auto rounded-xl bg-secondary-50 shadow-sm hover:shadow-2xl hover:bg-secondary-100 transition-transform duration-300 select-none">
      {/*전체카드 >  와인정보 + 설명*/}
      <div className="flex justify-between items-center gap-8">
        {/*와인정보 >  이미지 + 정보*/}
        <img src={images[1] ?? images[0]} alt={name} className="w-15" draggable="false" />
        <div className="flex flex-col justify-between align-baseline gap-1">
          {/*와인정보  : 영어이름+한국이름?+도수+향미료+맛(당도, 산미, 탄닌, 바디감)*/}
          <h3 className="text-2xl mb-4">{name.length < 32 ? name : name.slice(0, 32) + ' ... '}</h3>
          {/* <p>
            소테른<span>{alcohol_content ? `[${alcohol_content}]` : ''}</span>
          </p> */}
          <div className="flex items-center gap-1.5  whitespace-nowrap font-normal">
            <img
          ...
          <div className="flex items-center gap-1.5 mb-2 whitespace-nowrap font-normal">
